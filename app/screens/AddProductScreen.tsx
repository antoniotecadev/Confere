...
import CustomPhotoModal from "@/components/ui/CustomPhotoModal";
...
export default function AddProductScreen() {
  ...
  const [supermarketName, setSupermarketName] = useState<string | undefined>(
    undefined,
  );
  ...
  const prePrices = [
    50, 100, 150, 200, 250, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000,
  ...
  const loadSuggestions = async () => {
    const productSuggestions =
      await PriceComparisonService.suggestProducts(name);
  ...
  const validateForm = (): boolean => {
    ...
    if (price.trim()) {
      ...
      if (isNaN(priceValue) || priceValue <= 0) {
        Alert.alert(
          "Atenção",
          "Por favor, informe um preço válido maior que zero (ou deixa em branco para definir depois).",
        );
        return false;
      }
    }
  ...
  const analyzeProductLabel = (ocrResult: OcrResult): ProductData | null => {
    ...
    const enrichedBlocks = ocrResult.blocks
      .map((block: OcrBlock) => ({
    ...
    let priceBlock: ((typeof enrichedBlocks)[0] & { price: string }) | null =
      null;
    for (const block of enrichedBlocks) {
      ...
      if (textUpper.includes("KZ") || textUpper.includes("AKZ")) {
        ...
        const priceMatch = block.text.match(
          /(\d{1,3}(?:[.,]\d{3})*(?:[,]\d{2})?)\s*(?:akz|kz)/i,
        );
        if (priceMatch) {
  ...
  const takePictureAndOCR = async () => {
    ...
    try {
      ...
      const ocrResult: OcrResult = await MlkitOcr.recognizeText(
        photo.uri,
        "latin",
      );
  ...
  return (
    ...
    >
      ...
      <View style={styles.ocrButtonContainer}>
        <Pressable
          style={[
            styles.ocrToggleButton,
            isCameraActive && styles.ocrToggleButtonActive,
          ]}
        ...
        <View style={styles.cameraContainer}>
          ...
          <View style={styles.focusOverlay}>
            <Text style={styles.instructionText}>
              Centralize a etiqueta com o preço
            </Text>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.prePricesSection}>
            <View style={styles.prePricesHeader}>
              ...
              >
                <View
                  style={[styles.checkbox, isSumMode && styles.checkboxActive]}
                >
            ...
            parseInt(quantity) > 0 && (
              <View style={styles.totalPreview}>
                ...
                <Text style={styles.totalPreviewValue}>
                  {(parsePrice(price) * parseInt(quantity)).toLocaleString(
                    "pt-AO",
                    {
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.footerRow}>
          ...
          <Pressable
            style={[
              styles.footerMicBtn,
              voiceListening && styles.footerMicBtnActive,
            ]}
